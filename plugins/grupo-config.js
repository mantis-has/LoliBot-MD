...
if (args.length < 2) throw ` ${await tr("Formato incorrecto. Usa")}: !grupo [id/enlace] [ID/URL] - [acción] [+número si aplica]`
...
if (!inviteCode) throw `「🩵」 ${await tr("Enlace inválido. Usa un enlace de WhatsApp válido.")}`
try {
const inviteInfo = await conn.groupGetInviteInfo(inviteCode);
groupId = inviteInfo.id;
} catch (e) {
throw `「🩵」 ${await tr("No se pudo obtener información del grupo. Verifica el enlace o que el bot tenga acceso.")}`
...
throw `「🩵」 ${await tr("Debes proporcionar un enlace válido.")}`
}
const inviteCode = identifier.match(/(?:https:\/\/)?(?:www\.)?(?:chat\.|wa\.)?whatsapp\.com\/(?:invite\/|joinchat\/)?([0-9A-Za-z]{22,24})/i)?.[1];
if (!inviteCode) throw `「🩵」 ${await tr("Enlace inválido. Usa un enlace de WhatsApp válido.")}`
try {
...
throw `「🩵」 ${await tr(`Usa "id" o "enlace" como primer argumento, o pasa directamente un enlace válido.`)}`;
}} else if (m.isGroup) {
action = args[0]?.toLowerCase();
target = args[1]?.replace(/@/, '') + '@s.whatsapp.net';
}

if (!groupId) return m.reply(`「🩵」 ${await tr("Debes estar en un grupo o especificar un ID/enlace en privado.")}`);
const botId = conn.user.jid;
const groupMetadata = await conn.groupMetadata(groupId);
const isBotAdmin = groupMetadata.participants.some(p => p.id === botId && (p.admin === 'admin' || p.admin === 'superadmin'));
if (!isBotAdmin) return m.reply(`「🩵」 ${await tr("El bot debe ser admin para ejecutar este comando.")}`);
if (!action) throw `「🩵」 ${await tr("Debes especificar una acción")} (abrir, cerrar, daradmin, etc.).`
...
m.reply(`「🩵」 ${await tr("¡GRUPO CERRADO! Solo admins pueden escribir.")}`);
break;

case 'addadmin': case 'promote': case 'daradmin':
if (!target) throw `「🩵」 ${await tr("Especifica un número")} (${await tr("ejemplo")}: - daradmin +51987654321) ${await tr("o menciona en grupo.")}`
...
if (!target) throw `「🩵」 ${await tr("Especifica un número")} (${await tr("ejemplo")} - quitaradmin +51987654321)  ${await tr("o menciona en grupo.")}`
...
if (!target) throw `「🩵」 ${await tr("Especifica un número")} (${await tr("ejemplo")}: - eliminar +51987654321)  ${await tr("o menciona en grupo.")}.`
await conn.groupParticipantsUpdate(groupId, [target], 'remove');
m.reply(`「🩵」 @${target.split('@')[0]} ${await tr("ha sido eliminado del grupo.")}`);
break;

case "aprobar":
if (!target) throw `「🩵」 ${await tr("Especifica un número")} (${await tr("ejemplo")}: - aprobar +51987654321).`
